...
static const char *weekday_fr[] = {"DIM", "LUN", "MAR", "MER", "JEU", "VEN", "SAM"};
static const char *weekday_en[] = {"SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"};

static const char *get_day_name(int day_index, const char *lang) {
  if (day_index < 0 || day_index > 6) return "???";
  if (lang[0] == 'f' && lang[1] == 'r') return weekday_fr[day_index];
  return weekday_en[day_index];
}

#ifdef PBL_COLOR
// Build icon ID from text (version simplifiée, BW uniquement pour économiser mémoire)
static int build_icon_id(const char *text_icon) {
  if (!text_icon || text_icon[0] == '\0' || text_icon[0] == ' ') {
    return RESOURCE_ID_ENSOLEILLE_W;
  }
  if (strstr(text_icon, "clear") != NULL) return RESOURCE_ID_ENSOLEILLE_W;
  if (strstr(text_icon, "fair") != NULL) return RESOURCE_ID_FAIBLES_PASSAGES_NUAGEUX_W;
  if (strstr(text_icon, "partly") != NULL) return RESOURCE_ID_DEVELOPPEMENT_NUAGEUX_W;
  if (strstr(text_icon, "cloudy") != NULL) return RESOURCE_ID_FORTEMENT_NUAGEUX_W;
  if (strstr(text_icon, "rain") != NULL) return RESOURCE_ID_AVERSES_DE_PLUIE_FORTE_W;
  if (strstr(text_icon, "thunder") != NULL) return RESOURCE_ID_FORTEMENT_ORAGEUX_W;
  if (strstr(text_icon, "snow") != NULL || strstr(text_icon, "sleet") != NULL) {
    return RESOURCE_ID_NEIGE_FORTE_W;
  }
  if (strstr(text_icon, "fog") != NULL) return RESOURCE_ID_BROUILLARD_W;
  return RESOURCE_ID_ENSOLEILLE_W;
}
#endif

// Dessiner une ligne de prévision pour un jour
static void draw_day_row(GContext *ctx, const WeatherDaysData *d, int idx, int y) {
...
#ifdef PBL_COLOR
  // Icône météo (centre) - 24x24 - Only on color platforms to save heap on Aplite
